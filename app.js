...
async function checkDomain(btn){
 const d=btn.dataset.domain; btn.disabled=true; btn.classList.remove('free','taken','unknown'); btn.querySelector('small').textContent='Sjekker…';
 try{
  const r=await fetch('https://rdap.norid.no/domain/'+encodeURIComponent(d),{method:'GET'});
  if(r.status===200){btn.classList.add('taken');btn.querySelector('small').textContent='Opptatt';}
  else if(r.status===404){
   let j={};try{j=await r.json()}catch(_){}
   const msg=String(j.errorCode||'')+' '+String(j.title||'')+' '+String(j.description||'');
   if(/not available for registration/i.test(msg)){btn.classList.add('taken');btn.querySelector('small').textContent='Ikke tilgjengelig';}
   else{btn.classList.add('free');btn.querySelector('small').textContent='Ser ledig ut';}
  }else{throw Error('HTTP '+r.status)}
 }catch(e){btn.classList.add('unknown');btn.querySelector('small').textContent='Sjekk hos Norid';window.open('https://www.norid.no/no/domeneoppslag/','_blank');}
